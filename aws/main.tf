...
module "alb_visualizer" {
  count             = var.visualizer_enabled ? 1 : 0
  source            = "./alb"
  name              = "${var.vpc_name != "" ? var.vpc_name : ""}-visualizer"
  internal          = true
  vpc_id            = local.vpc_id_rule
  subnets           = local.subnets_rule
  backend_port      = 8050
...
module "alb_sig_provider" {
  count             = var.sig_provider_enabled ? 1 : 0
  source            = "./alb"
  name              = "${var.vpc_name != "" ? var.vpc_name : ""}-sig-provider"
  internal          = true
  vpc_id            = local.vpc_id_rule
  subnets           = local.subnets_rule
  backend_port      = 8050
...
module "alb_stats" {
  count             = var.stats_enabled ? 1 : 0
  source            = "./alb"
  name              = "${var.vpc_name != "" ? var.vpc_name : ""}-stats"
  internal          = true
  vpc_id            = local.vpc_id_rule
  subnets           = local.subnets_rule
  backend_port      = 8050
...
module "alb_eth_bytecode_db" {
  count             = var.eth_bytecode_db_enabled ? 1 : 0
  source            = "./alb"
  name              = "${var.vpc_name != "" ? var.vpc_name : ""}-eth-bytecode-db"
  internal          = true
  vpc_id            = local.vpc_id_rule
  subnets           = local.subnets_rule
  backend_port      = 8050
